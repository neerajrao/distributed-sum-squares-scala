...
object Listener1 extends ListenerTemplate{
  ...
  var port_no: Int = Configuration.port1 //default
  var name: Symbol = Configuration.name1 //default
        ...
        case msgFromSpeaker : (Int,Int,Int,Double) => {//(startAt, range, K, cSum)
          ...
          for (i <- 0 until numActors) {
            ...
            if (K % 2 == 0) { //even
              ...
              Listener1.calculatorsArray(i) ! (((i*V)+K/2+0.5)+startAt-1)
            }
            else{ //odd
              //println("Sending "+(((i*V)+K/2+1)*1.0))
              Listener1.calculatorsArray(i) ! ((((i*V)+K/2+1)*1.0)+startAt-1)
            }
          }
        }
