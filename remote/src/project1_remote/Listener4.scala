...
object Listener4 extends ListenerTemplate{
  ...
  var port_no: Int = Configuration.port4 //default
  var name: Symbol = Configuration.name4 //default
        ...
        case msgFromSpeaker : (Int,Int,Int,Double) => {//(startAt, range, K, cSum)
          ...
          for (i <- 0 until numActors) {
            ...
            if (K % 2 == 0) { //even
              ...
              Listener4.calculatorsArray(i) ! (((i*V)+K/2+0.5)+startAt-1)
            }
            else{ //odd
              //println("Sending "+(((i*V)+K/2+1)*1.0))
              Listener4.calculatorsArray(i) ! ((((i*V)+K/2+1)*1.0)+startAt-1)
            }
          }
        }
