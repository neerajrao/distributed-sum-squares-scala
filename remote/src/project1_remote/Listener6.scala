...
object Listener6 extends ListenerTemplate{
  ...
  var port_no: Int = Configuration.port6 //default
  var name: Symbol = Configuration.name6 //default
        ...
        case msgFromSpeaker : (Int,Int,Int,Double) => {//(startAt, range, K, cSum)
          ...
          for (i <- 0 until numActors) {
            ...
            if (K % 2 == 0) { //even
              ...
              Listener6.calculatorsArray(i) ! (((i*V)+K/2+0.5)+startAt-1)
            }
            else{ //odd
              //println("Sending "+(((i*V)+K/2+1)*1.0))
              Listener6.calculatorsArray(i) ! ((((i*V)+K/2+1)*1.0)+startAt-1)
            }
          }
        }
